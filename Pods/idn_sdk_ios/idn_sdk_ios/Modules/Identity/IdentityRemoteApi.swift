...
open class IdentityRemoteApi  {
    ...
    init(httpClient: AnyObject) {
        ...
        accountKitUrl = "\(baseIdentityUrl)/auth/social/accountKit/authorize"
    }
    ...
    func accountKitLogin(loginDetails : Dictionary<String, Any>, completionHandler: @escaping (AnyObject?, Int, NSError?) -> ()){
        
        let hostUrl : String = apiContext.getHost("app")
        let connectUrl = "\(hostUrl)\(accountKitUrl)"
        
        
        networkAPI.post(connectUrl, paramsDict: loginDetails, headers: clientTokenheaders, completionHandler: completionHandler)
